...
# Main Effects:
# 1. Switch Costs
DT_Switch_Cost = data.table(SC = c("Switch Cost","Switch Cost"), Experiment = c("Experiment 1", "Experiment 2"), fit = c(-0.414, -0.316), lower = c(-0.487, -0.387), upper = c(-0.340, -0.245)) 
DT_Switch_Cost_Threshold = data.table(SC = c("Switch Cost","Switch Cost"), Experiment = c("Experiment 1", "Experiment 2"), fit = c(0.063, 0.1168), lower = c(0.029, 0.083), upper = c(0.096, 0.150)) 
...
# 3.Congruency
DT_congruency = data.table(SC = c("congruency","congruency"), Experiment = c("Exp1", "Exp2"), fit = c(-0.480141688309, -0.374055233136), lower = c(-0.40468217, -0.3134809), upper = c(-0.55646391, -0.43782279)) # p value < 0.001 Exp2 p = 0.4734
DT_congruency_threshold = data.table(SC = c("congruency","congruency"), Experiment = c("Exp1", "Exp2"), fit = c(-0.101363625202, -0.0979685862689), lower = c(-0.0728155, -0.06767293), upper = c(-0.1318213,-0.13013283)) # p value < 0.001 & p value < 0.001

# Interactions:
# 1. Asymmetric Switch Costs
DT_SC_Task = data.table(SC = c("",""), Experiment = c("Experiment 1: p < 0.001", "Experiment 2: p = 0.627"), fit = c(-0.281, 0.02166), lower = c(-0.423, -0.12), upper = c(-0.132, 0.1538)) 
DT_SC_Task_Threshold = data.table(SC = c("",""), Experiment = c("Experiment 1", "Experiment 2"), fit = c(-0.0267, 0.138670287342), lower = c(-0.0789, 0.08645048), upper = c(0.0246, 0.19032629)) 

# 2. Switch Costs : Response Congruency
DT_Switch_Cost_Congruency = data.table(SC = c("",""), Experiment = c("Experiment 1: p < 0.001", "Experiment 2: p < 0.001"), fit = c(-0.272, -0.2023), lower = -c(0.154, 0.0939), upper = -c(0.390, 0.332))  
DT_Switch_Cost_Congruency_Threshold = data.table(SC = c("",""), Experiment = c("Experiment 1: p = 0.003", "Experiment 2: p = 0.031"), fit = c(-0.0668, -0.0526), lower = -c(0.014, -0.00306337), upper = -c(0.118, 0.11610236))  
```
